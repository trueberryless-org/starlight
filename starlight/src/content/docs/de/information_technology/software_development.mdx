...
<Card title="Programmiersprachen" icon="seti:json">
    Wähle die Programmiersprache, welche verwendet werden soll, um die Inhalte zu erklären:
    <Tabs>
        <TabItem label="CSharp" icon="seti:c-sharp">
            C# ist eine objektorientierte Programmiersprache, die von Microsoft entwickelt wurde.
            Ihre besonderen Features umfassen eine starke Typisierung, eine umfangreiche
            Standardbibliothek und die Unterstützung von LINQ (Language Integrated Query) für
            einfache bis komplexere Datenverarbeitungen.
        </TabItem>
        <TabItem label="Rust" icon="seti:rust">
            Rust ist eine Systemprogrammiersprache, die von Mozilla Research entwickelt wurde. Sie
            zeichnet sich durch eine hohe Leistung, Speichersicherheit ohne Garbage Collection und
            einen innovativen Borrowing-Mechanismus aus, der sicherstellen soll, dass kein
            undefiniertes Verhalten auftritt.
        </TabItem>
    </Tabs>
</Card>
...
Datentypen sind die Grundlage für die Programmierung. Sie ermöglichen das Speichern von Daten, wie zum Beispiel das Alter, einen Namen oder eine Adresse. Dabei muss man unterscheiden, dass es verschiedene Typen von Daten gibt, wie zum Beispiel Nummern, Buchstaben, Listen, ...
...
#### Konzept: Generalisierung und Spezialisierung

#### Konzept: Abstraktion

#### Konzept: Identität

#### Konzept: Datenkapselung
...
Wenn mehr und mehr Anfragen (steigende Last) auf eine Applikation kommen, muss diese skaliert werden. Die Skalierung der Hardware hat hierbei einen bestimmten Pivot-Punkt, ab welchem das Hinzufügen von Rechnereinheiten keinen Vorteile bezüglich Aufallsicherheit mehr bringen und das system nicht mehr skalierbar ist. Aus diesem Grund gibt es verschiedene Möglichkeiten der Skalierung.

##### Skalierung durch Aufteilung unterschiedlicher Elemente

Eine Art, Software zu skalieren schlägt vor, unterschiedliche Elemente auf mehrere Rechner aufzuteilen. Beispielsweise kann man ein System aufbauen, welches mehrere Datenbanken skaliert und verschiedenen Einträge auf verschiedenen Instanzen verarbeitet. Alle Nutzer, dessen Name zwischen `A` und `F` liegt, werden auf in der ersten Datenbank gespeichert, alle Nutzer, dessen Name zwischen `G` und `L` liegt, in der zweiten Instanz, usw.

##### Skalierung durch Aufteilung ähnlicher Elemente

Software-Skalierung kann auch mittels der Aufteilung gleicher oder ähnlicher Elemente geschehen. Hierbei spricht man meistens von einer `Microservice-Architecture` oder allgemein einer `Service-Oriented Architecture` (`SOA`). Denn diese Architekturen haben die Geschäftslogik, Datenbankinteraktion und API-Services in verschiedene kleine Komponenten aufgeteilt, sodass nicht der gesamte Code der Applikation ein `Monolith` ist. Dadurch können die einzelnen Services unabhängig voneinander skaliert werden und man hat bei besonders dezentralen Systemen den zusätzlichen Vorteil der Entkoppelung des inhärenten Systems.

##### Skalierung durch Klonen

Die herkömmliche Methode der Skalierung ist das einfache Klonen der physischen Einheiten. Diese Art der Skalierung erreich jedoch ab einem bestimmten Maß an Last an Anfragen einen Punkt, an dem die Skalierung nicht mehr sinnvoll ist. Die beiden anderen Methoden müssen dann verwendet werden, um die Last zu bewältigen.
...
In der Softwareentwicklung kommt es oft vor, dass ähnliche Problemstellungen gelöst werden müssen. Um bereits durchdachte Lösungen für diese Probleme effizienter umsetzen und entwickeln zu können, gibt es **Design Pattern**.
...
Gestalterische Entwurfsmuster beschäftigen sich mit Problemstellungen beim Erstellen von Objekten.

#### Abstract Factory

Die Abstract Factory ist ein gestalterisches Entwurfsmuster, welches Familien von verwandten Objekten erstellen kann, ohne die konkreten Klassen zu kennen.

##### Problem

Stellen Sie sich vor, dass Sie einen Onlineshop erstellen sollen, wo man Kleider einkaufen kann. Die Besonderheit des Onlineshops ist, dass sich ein neuer Kunde beim Registrieren für eine Herstellermarke entscheiden soll und anschließend auf der gesamten Website nur mit Kleidern dieser Marke interagieren kann. Da Sie klein anfangen, gibt es nur die beiden Marken `Valentino` und `Gucci` und die beiden Kleidungsstücke `Shirt` und `Pant`. Damit dargestellt wird, dass auch Funktionalitäten untereinander einfach möglich sind, soll man recht einfach von einer Hose das passende Hemd bekommen können.

Die Website hat eigentlich zwei Herausforderungen: Für Kunden, welche eine Marke gewählt haben, soll der Onlineshop nur diese Marke anbieten. Außerdem soll aufgrund des ständigen Austausches des Angebots die Klassen nicht immer adaptiert werden müssen.

##### Lösung

Eine Abstract Factory bietet eine ideale Lösung für dieses Problem. Als erstes werden alle Funktionalitäten der Kleidungsstücke in Interfaces definiert. Das bedeutet, dass es ein Interface für `Shirt` und ein Interface für `Pant` gibt.
...
pub trait Pant {
    fn add_to_cart(&self);
    fn matching_shirt(&self) -> Box<dyn Shirt>;
}
```
...
Nun definieren wir ein Interface für die `Factory`, damit jede Factory, egal ob `ValentinoFactory` oder `GucciFactory`, die gleichen Methoden zur Verfügung stellt.
...
```rust
pub trait ClothingFactory {
    fn new_shirt(&self) -> Box<dyn Shirt>;
    fn new_pant(&self) -> Box<dyn Pant>;
}
```
...
Anschließend lassen wir die einzelnen Klassen und Fabriken von diesen Interfaces richtig erben und implementieren die notwendigen Methoden.

<Tabs>
<TabItem label="CSharp" icon="seti:c-sharp">

```cs
class ValentinoShirt : IShirt
{
    public string AddToCart()
    {
        return "Add Valentino Shirt to Cart";
    }
}
```

```cs
class GucciShirt : IShirt
{
    public string AddToCart()
    {
        return "Add Gucci Shirt to Cart";
    }
}
```

```cs
class ValentinoPant : IPant
{
    public string AddToCart()
    {
        return "Add Valentino Pants to Cart";
    }

    public IShirt MatchingShirt()
    {
        return new ValentinoShirt();
    }
}
```

```cs
class GucciPant : IPant
{
    public string AddToCart()
    {
        return "Add Gucci Pants to Cart";
    }
...
```cs
class ValentinoFactory : IFactory
{
    public IShirt NewShirt()
    {
        return new ValentinoShirt();
    }

    public IPant NewPant()
    {
        return new ValentinoPant();
    }
}
```

```cs
class GucciFactory : IFactory
{
    public IShirt NewShirt()
    {
        return new GucciShirt();
...
```rust
pub struct ValentinoShirt;

impl Shirt for ValentinoShirt {
    fn add_to_cart(&self) {
        println!("Add Valentino Shirt to Cart");
    }
}
```

```rust
pub struct GucciShirt;

impl Shirt for GucciShirt {
    fn add_to_cart(&self) {
        println!("Add Gucci Shirt to Cart");
    }
}
```

```rust
pub struct ValentinoPant;

impl Pant for ValentinoPant {
    fn add_to_cart(&self) {
        println!("Add Valentino Pants to Cart");
    }

    fn matching_shirt(&self) -> Box<dyn Shirt> {
        Box::new(ValentinoShirt)
    }
}
```

```rust
pub struct GucciPant;

impl Pant for GucciPant {
    fn add_to_cart(&self) {
        println!("Add Gucci Pants to Cart");
    }
...
```rust
pub struct ValentinoFactory;

impl ClothingFactory for ValentinoFactory {
    pub fn new_shirt(&self) -> Box<dyn Shirt> {
        Box::new(ValentinoShirt)
    }

    pub fn new_pant(&self) -> Box<dyn Pant> {
        Box::new(ValentinoPant)
    }
}
```

```rust
pub struct GucciFactory;

impl ClothingFactory for GucciFactory {
    pub fn new_shirt(&self) -> Box<dyn Shirt> {
        Box::new(GucciShirt)
    }
...
Nun können wir die Klassen verwenden, ohne die konkreten Klassen kennen zu müssen.
...
class Program
{
    ...
    {
        // Choose a factory per customer
        TestFactoryMethod(new ValentinoFactory());
        TestFactoryMethod(new GucciFactory());
    }

    // Only using interfaces
    public void TestFactoryMethod(IFactory factory)
    {
        IPant pant = factory.NewPant();
        IShirt matchingShirt = pant.MatchingShirt();

        Console.WriteLine(matchingShirt.AddToCart());
    }
}
```
...
fn main() {
    // Choose a factory per customer
    test_factory_method(ValentinoFactory {});
    test_factory_method(GucciFactory {});
}

fn test_factory_method(factory: Box<dyn ClothingFactory>) {
    let pant = factory.new_pant();
    let matching_shirt = pant.matching_shirt();

    println!("{}", matching_shirt.add_to_cart());
}
```
...
Singleton ist ein gestalterisches Entwurfsmuster, welches garantiert, dass nur eine einzige Instanz einer Klasse auf globaler Programmebene existiert.

##### Problem

In manchen Fällen benötigt man im gesamten Program eine globale Instanz einer Klasse. Beispielsweise ist es nicht notwendig, mehrere Instanzen einer Klasse zu erstellen, welche Datenbankabfragen durchführt. Doch wie stellt man sicher, dass es nur eine einzige Instanz der Klasse gibt?
...
Strukturelle Entwurfsmuster beschäftigen sich mit dem Aufbau von Klassen in großen Strukturen, damit diese Strukturen flexible und effizient bleiben. Meistens schlagen die strukturellen Entwurfsmuster die Vermeidung von unnötigen Abfragen von Datentypen vor, indem auf die Vererbung zurückgegriffen wird.
