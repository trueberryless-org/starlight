...
Die `Factory`-Methode ist ein Entwurfsmuster zur Erzeugung von Objekten, das eine Schnittstelle in einer Basisklasse bereitstellt, um Objekte zu erstellen, während es Unterklassen ermöglicht, den Typ der zu erstellenden Objekte zu ändern.

##### Problem

Angenommen, Sie entwickeln eine Anwendung für das Logistikmanagement. Die erste Version Ihrer App kann nur den Transport per LKW verwalten, daher liegt der Großteil des Codes in der `Truck`-Klasse.

Mit der Zeit wird Ihre App sehr beliebt. Täglich erhalten Sie Dutzende von Anfragen von Seetransportunternehmen, um die Seelogistik in die App zu integrieren.

Derzeit ist der Großteil des Codes an die `Truck`-Klasse geknüpft. Das Hinzufügen von Schiffen zur App würde eine Änderung des gesamten Codebestands erfordern. Außerdem müssten wahrscheinlich alle diese Änderungen erneut vorgenommen werden, wenn Sie später entscheiden, einen weiteren Typ von Transport in die App aufzunehmen.

Das Ergebnis wäre ziemlich unübersichtlicher Code, voller bedingter Anweisungen, die das Verhalten der App je nach Klasse von Transportobjekten steuern.

##### Lösung

Die Factory-Methode schlägt vor, direkte Objekterstellungsaufrufe (mit dem `new`-Operator) durch Aufrufe einer speziellen Factory-Methode zu ersetzen. Die Objekte werden immer noch mit dem `new`-Operator erstellt, aber er wird von innerhalb der Factory-Methode aufgerufen. Von einer Factory-Methode zurückgegebene Objekte werden oft als Produkte bezeichnet.

{/* Image */}

Auf den ersten Blick mag diese Änderung sinnlos erscheinen: Wir haben den Konstruktoraufruf nur von einem Teil des Programms in einen anderen verschoben. Allerdings können Sie jetzt die Factory-Methode in einer Unterklasse überschreiben und die Klasse der von der Methode erstellten Produkte ändern.

Es gibt jedoch eine Einschränkung: Unterklassen können nur verschiedene Arten von Produkten zurückgeben, wenn diese Produkte eine gemeinsame Basisklasse oder ein gemeinsames Interface haben. Außerdem sollte die Factory-Methode in der Basisklasse ihren Rückgabetyp als dieses Interface deklariert haben.

{/* Image */}

Beispielsweise sollten sowohl die Klassen `Truck` als auch `Ship` das Transport-Interface implementieren, das eine Methode namens `deliver()` deklariert. Jede Klasse implementiert diese Methode auf unterschiedliche Weise: Lastwagen liefern Fracht auf dem Landweg, Schiffe auf dem Seeweg. Die Factory-Methode in der Klasse `RoadLogistics` gibt LKW-Objekte zurück, während die Factory-Methode in der Klasse `SeaLogistics` Schiffe zurückgibt.

{/* Image */}

Der Code, der die Factory-Methode verwendet (oft als `Client-Code` bezeichnet), sieht keinen Unterschied zwischen den tatsächlichen Produkten, die von verschiedenen Unterklasse zurückgegeben werden. Der Client behandelt alle Produkte als abstrakten Transport. Dem Client ist bekannt, dass alle Transportobjekte die `deliver()`-Methode haben sollen, aber wie genau sie funktioniert, ist für den Client nicht wichtig.
